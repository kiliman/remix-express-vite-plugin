...
// Copied from https://github.com/remix-run/remix/blob/main/packages/remix-express/server.ts
...
export function createRemixRequest(
  ...
  let [hostname, hostnamePort] =
    (req.get('X-Forwarded-Host') ?? req.get('Host') ?? '').split(':') ?? []
  let [, hostPort] = req.get('host')?.split(':') ?? []
  let port = hostnamePort || hostPort
  let resolvedHost = `${hostname ?? req.hostname}${port ? `:${port}` : ''}`
